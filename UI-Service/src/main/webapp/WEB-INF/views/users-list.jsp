...
<div class="container mt-5">
    <h2>All Users</h2>
    <a href="${pageContext.request.contextPath}/ui/users/add" class="btn btn-success mb-3">Add New User</a>
    <table class="table table-bordered table-striped">
        <thead>
        <tr>
            <th>ID</th>
        ...
        <c:forEach var="user" items="${users}">
            <tr>
                ...
                <td>${user.email}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/ui/users/edit/${user.id}" class="btn btn-primary btn-sm">Edit</a>
                    <form action="${pageContext.request.contextPath}/ui/users/delete/${user.id}" method="post" style="display:inline">
                        <input type="hidden" name="_method" value="delete"/>
                        <button type="submit" class="btn btn-danger btn-sm">Delete</button>
                    </form>
                </td>
