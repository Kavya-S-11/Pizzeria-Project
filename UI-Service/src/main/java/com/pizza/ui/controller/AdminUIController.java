...
import com.pizza.ui.dto.LoginRequest;
...
public class AdminUIController {
 ...
 public String showProfile(@RequestParam String token, Model model) {
     ...
     model.addAttribute("admin", admin);
     model.addAttribute("jwtToken", token);
     return "admin-profile"; // JSP page for profile
 }
 ...
 public String updateProfile(@RequestParam String token,
                             ...
                             Model model) {
     try {
         // call admin-service to update profile
         AdminDTO updatedAdmin = adminClient.updateAdmin(adminDTO.getId(), adminDTO, token);

         model.addAttribute("admin", updatedAdmin);
         model.addAttribute("jwtToken", token);
         model.addAttribute("success", "Profile updated successfully!");

         return "admin-profile"; // show updated profile
     } catch (Exception e) {
         model.addAttribute("admin", adminDTO);
         model.addAttribute("jwtToken", token);
         model.addAttribute("error", "Failed to update profile. Please try again.");
         return "admin-profile"; // back to profile page with error
     }
 }
